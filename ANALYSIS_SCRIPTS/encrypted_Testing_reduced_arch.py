...
class DataLoader_:
    ...
    def load_and_prepare(config: ReducedNetworkTrainingConfig, hospital_id: str = None):
        ...
        try:
            ...
            # Filter by hospital if in federated mode (use positional indices)
            if hospital_id and assignments:
                # Get hospital assignments (assignment file is already aligned with data)
                train_hospital_mask = (assignments['train']['hospital'] == hospital_id).values
                test_hospital_mask = (assignments['test']['hospital'] == hospital_id).values
                
                # Apply masks to data
                X_train = X_train[train_hospital_mask]
                y_train = y_train[train_hospital_mask]
                X_test = X_test[test_hospital_mask]
                y_test = y_test[test_hospital_mask]
