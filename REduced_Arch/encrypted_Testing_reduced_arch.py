...
class DataLoader_:
    ...
    @staticmethod
    def load_and_prepare(config: ReducedNetworkTrainingConfig):
        """
...
class ModelTrainer:
    ...
    @staticmethod
    def train_model(config: ReducedNetworkTrainingConfig, data: Dict):
        """
...
def main():
    ...
    print("For Privacy-Preserving Encrypted Inference")
    print("=" * 100)
    ...
    data = DataLoader_.load_and_prepare(config)
    
    # Train model
    model, history = ModelTrainer.train_model(config, data)
    ...
    # Generate report
    print("\n[Generating Report]")
    report_path = config.REPORTS_DIR / "phase_3_retrain_reduced_network_report.txt"
    ReportGenerator.generate_report(config, metrics, history, report_path)
    
    # Plot training curves
    plot_path = config.REPORTS_DIR / "phase_3_training_curves.png"
    ReportGenerator.plot_training_history(history, plot_path)
    ...
    metrics_path = config.REPORTS_DIR / "phase_3_retrain_metrics.json"
    ...
    print(f"\n📁 Artifacts:")
    print(f"  Model: {config.MODELS_DIR}/mlp_best_model_REDUCED.pt")
    print(f"  Report: {report_path}")
    print(f"  Plots: {plot_path}")
    print(f"  Metrics: {metrics_path}")
    
    print(f"\n✅ READY FOR PHASE 6c!")
    print(f"   Use this model in encrypted inference")
    print(f"   No more 'scale out of bounds' errors!")
    print(f"   Predictions will be valid!")
