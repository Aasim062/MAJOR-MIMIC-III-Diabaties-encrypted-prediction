...
class DataLoader:
    ...
    @staticmethod
    def load_hospital_specific_test_data(
        hospital_id: str,
        config: Phase6cConfig
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        Load hospital-specific test data by filtering with assignment_test.csv
        ...
        Returns:
            (X_test_hospital, y_test_hospital) numpy arrays
        """
        ...
        try:
            ...
            assignment_df = pd.read_csv(config.DATA_DIR / "assignment_test.csv")
            
            if config.VERBOSE:
                print(f"  Loaded assignment data: {len(assignment_df)} records")
            
            # Filter for this hospital
            hospital_mask = assignment_df['hospital'] == hospital_id
            hospital_indices = assignment_df[hospital_mask].index.tolist()
            
            X_test_hospital = X_test[hospital_indices]
            y_test_hospital = y_test[hospital_indices]
            
            if config.VERBOSE:
                print(f"  ✓ Filtered for Hospital {hospital_id}")
                print(f"    Samples: {len(X_test_hospital):,}")
                print(f"    Features: {X_test_hospital.shape[1]}")
                if len(y_test_hospital) > 0:
                    print(f"    Positive rate: {y_test_hospital.mean():.4f}")
            
            return X_test_hospital, y_test_hospital
            
        except Exception as e:
            print(f"  ❌ Error loading hospital-specific data: {e}")
            raise
...
class HospitalInferenceExecutor:
    ...
    def run_inference(
        ...
        # ===== STEP 2: Load hospital-specific test data =====
        if config.VERBOSE:
            print(f"\n[Step 2: Load Hospital-Specific Test Data]")
        
        try:
            X_test, y_test = DataLoader.load_hospital_specific_test_data(hospital_id, config)
            
            if len(X_test) == 0:
                print(f"  ⚠️ No test samples for Hospital {hospital_id}")
                return None
